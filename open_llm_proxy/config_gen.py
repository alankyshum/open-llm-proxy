...
def _validate_account_in_token(token: str, context: str) -> None:
    ...
    if "@" in provider_part:
        ...
        if not account or not _ACCOUNT_RE.match(account):
            raise ValueError(f"Malformed account tag in '{token}' in {context}")
...
def map_token_to_deployment_params(token: str) -> dict:
    ...
    if base_provider == "claude-cli":
        ...
        result: dict = {"model": f"claude-cli/{rest}"}
        ...
        return {"model": f"openrouter/{rest}", "api_key": "os.environ/OPENROUTER_API_KEY"}
        ...
        return {"model": f"gemini/{model_id}", "api_key": "os.environ/GEMINI_API_KEY"}
        ...
        return {"model": f"{base_provider}/{rest}"}
    ...
    for raw_model in sorted(model_strings):
        ...
        if is_bracketed:
            ...
            if len(tokens) > 1:
                ...
                fallbacks.append({internal_alias: tokens[1:]})
        else:
        ...
        # They don't have open-llm-proxy/ prefix because they come from supported_models list directly  # intentional long protocol text or compatibility message  # noqa: E501
        # But since we added "open-llm-proxy/{m}" to model_strings, they've already been processed above  # intentional long protocol text or compatibility message  # noqa: E501
...
def generate_config(agent_config_path: str) -> dict:
    return generate_config_from_data(parse_agent_config(Path(agent_config_path).read_bytes()))


def main():
    parser = argparse.ArgumentParser(
        description="Generate LiteLLM Router config from agent-config.yml"
    ...
    config_path = find_agent_config(args.config_path)
