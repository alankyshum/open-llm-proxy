...
def test_launch_server_ui_default_on_degrades_without_db(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "open_llm_proxy.server_launcher.setup_callbacks", lambda *args, **kwargs: None
    )
    monkeypatch.setattr(
        "open_llm_proxy.server_launcher.generate_config_from_data", lambda *args, **kwargs: {}
    )
...
def test_ui_command_success(monkeypatch, capsys):
    ...
    assert code == 0
    assert (
        "Success! LiteLLM Admin UI is running and responding with 200 OK."
...
def test_ui_command_disabled_middleware(monkeypatch, capsys):
    ...
    assert (
        "Error: The server responded, but the Admin UI is disabled (middleware block)." in err_out
    )
...
def test_ui_command_http_error(monkeypatch, capsys):
    ...
    err_out = capsys.readouterr().err
    assert (
        "Error: Admin UI not found (404) at http://127.0.0.1:8765/ui. Is the UI disabled or port incorrect?"
        in err_out
    )
