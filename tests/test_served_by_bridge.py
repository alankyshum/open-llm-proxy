...
def test_attribution_id_from_litellm_request_metadata():
    ...
    data = {
        "litellm_params": {
            "proxy_server_request": {"headers": {"X-Open-LLM-Proxy-Attribution-ID": uid}}
        }
    }
...
def test_internal_attribution_endpoint(monkeypatch, tmp_path):
    ...
    monkeypatch.delenv("OPEN_LLM_PROXY_ATTRIBUTION_TOKEN", raising=False)
    response = client.get(
        f"/internal/attribution/v1/{uid}", headers={"Authorization": "Bearer secret"}
    )
    ...
    response = client.get(
        f"/internal/attribution/v1/{uid}", headers={"Authorization": "Bearer bad-token"}
    )
    assert response.status_code == 401

    # 4. Authorized but ID doesn't exist
    response = client.get(
        f"/internal/attribution/v1/{uid}", headers={"Authorization": "Bearer super-secret-token"}
    )
    assert response.status_code == 404

    # 5. Invalid UUID format
    response = client.get(
        "/internal/attribution/v1/not-a-uuid",
