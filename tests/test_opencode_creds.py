...
def test_get_opencode_api_key_from_file(monkeypatch, tmp_path):
    ...
    auth_file.write_text(
        json.dumps({"opencode": {"type": "api", "key": "sk-opencode-file456"}}), encoding="utf-8"
    )
...
def test_get_opencode_api_key_precedence(monkeypatch, tmp_path):
    ...
    auth_file.write_text(
        json.dumps({"opencode": {"type": "api", "key": "sk-opencode-file-loses"}}), encoding="utf-8"
    )
...
def test_get_opencode_api_key_malformed_json_details(monkeypatch, tmp_path):
    ...
    auth_file.write_text('{"opencode": {"key": "DUMMY-NOT-A-SECRET-key"}', encoding="utf-8")
    ...
    assert "DUMMY-NOT-A-SECRET-key" not in err_msg
